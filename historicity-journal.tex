...
interpolation candidates.  The mythic-origin response to these
passages \textit{cannot} therefore rely on text-critical or
stylometric excision.  It must rely on semantic and contextual
analysis.

\paragraph{An asymmetry in patristic reception.}
A systematic examination of the Apostolic Fathers
(Lightfoot collection, c.~95--150~CE) reveals a striking asymmetry
in which Pauline passages are cited by the earliest extant
Christian writers outside the New Testament.
\textbf{Romans~1:3--4} (\grk{ek spermatos Dauid}) is cited by
Ignatius of Antioch (c.~107--110~CE) in five separate letters
(\textit{Ephesians}~18.2 and 20.2, \textit{Trallians}~9.1,
\textit{Romans}~7.3, \textit{Smyrnaeans}~1.1) and is deployed as
load-bearing evidence in his anti-docetic polemic establishing
Jesus's true embodiment.  The passage is textually stable by
c.~110~CE.  \textbf{Galatians~4:4} is echoed in Ignatius
\textit{Smyrnaeans}~1.1 (\enquote{truly born of a virgin}) in a
parallel anti-docetic context.

\textbf{Galatians~1:19} (\enquote{James, the brother of the
Lord}), by contrast, is \textit{not cited or paraphrased by any
Apostolic Father}.  The name \enquote{James} does not appear in
the entire collection.  \textbf{1~Corinthians~15:3--8}
(the resurrection witness list) is likewise not cited---1~Clement
47.1 explicitly cites 1~Corinthians~1 but does not cite the
witness list.  The first clear patristic citations of Gal~1:19
come with Tertullian's \textit{Adversus Marcionem} (c.~207~CE)
and Jerome's \textit{Adversus Helvidium} (c.~390~CE).

The silence does not prove these passages were absent
(P46 attests both), but it does establish that the earliest
surviving Christian writers did not treat Gal~1:19 as evidence
for Jesus's biological family in contexts (Ignatius's anti-docetic
argument) where it would have served their case.  The
historicist reading of Gal~1:19 as a biographical datum about
Jesus's family must account for the fact that Ignatius, Polycarp,
1~Clement, Barnabas, and the other Apostolic Fathers do not so
deploy it.
