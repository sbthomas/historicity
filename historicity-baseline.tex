...
Corpus & A-rate & 95\% CI \\
\midrule
Baseline (non-Christian, $N=212$)  & 0.9\%  & [0.3\%, 3.4\%] \\
Paul (undisputed, $N=121$)         & 96.7\% & [91.8\%, 98.7\%] \\
\midrule
\textbf{Ratio (Paul / baseline)}   & \textbf{$\approx 103\times$} & (CIs do not overlap) \\
\bottomrule
\end{tabular}
\end{center}

The Pauline A-rate exceeds the non-Christian baseline by approximately
two orders of magnitude.  The confidence intervals do not overlap at
any reasonable significance level: Paul's 95\% CI lower bound is
91.8\%, while the baseline 95\% CI upper bound is 3.4\%.  This is a
genuinely extreme distributional deviation.
...
1 Cor 9:5), the conservative Bayesian framing is:
\begin{itemize}[leftmargin=1.5em]
\item The \textit{general} prior (from the non-Christian baseline)
  would favour biological-sibling reading by roughly $77$ to $1$
  against community reading for any given \textit{adelphos} token.
\item The \textit{Pauline-specific} prior reverses that ratio to
  roughly $29$ to $1$ favouring community reading.
\item Which prior applies depends on whether Paul's \textit{adelphos}
  usage is governed by (a) general Greek conventions or (b) his own
  attested distributional pattern.  The evidence overwhelmingly favours
  (b): Paul's distribution is stable across seven letters with 121
  tokens and 0 unambiguous biological uses outside the two disputed
  passages.
\end{itemize}
...
\item \textbf{Inscriptions excluded.}  Greek inscriptions (IG, SEG) are
  a relevant seventh register — cult-association, civic-brotherhood,
  and guild uses cluster there.  They were not sampled in this
  iteration due to digital-access constraints.  Their inclusion would
  plausibly raise the baseline A-rate, most likely into the 3--10\%
  range.  The A-rate gap would still be enormous.
...
from six non-Christian, contemporary-or-earlier Greek corpora, the
rate of community / voluntary-association usage is $0.9\%$ (95\% CI:
$[0.3\%, 3.4\%]$).  The comparable rate in the seven undisputed Pauline
letters is $96.7\%$ (95\% CI: $[91.8\%, 98.7\%]$).  The ratio is
approximately $103\times$, and the confidence intervals do not overlap.

The conservative Bayesian reading is that Paul's \textit{adelphos}
usage is a registered dialect of early Christian community discourse,
strikingly distinctive against the baseline of contemporary Greek.  Any
interpretation of a disputed Pauline \textit{adelphos} token --- notably
Galatians 1:19 --- should be anchored in Paul's own attested
distribution, not in general Greek usage conventions.  That anchoring
is what the main paper's §3.5 argues for on direct lexical grounds; the
present supplement corroborates it quantitatively.
